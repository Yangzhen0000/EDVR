...
class SDR4kDataset(data.Dataset):
    '''
    Reading the training SDR4k dataset
    key example: 00000000_000
    GT: Ground-Truth;
    ...
    def __init__(self, opt):
        super(SDR4kDataset, self).__init__()
        self.opt = opt
    ...
    def _read_img_mc(self, path):
        ...
        # Return BGR HWC [0, 65535], uint16
        value = mc.pyvector()
        self.mclient.Get(path, value)
        value_buf = mc.ConvertBuffer(value)
        # img_array = np.frombuffer(value_buf, np.uint8)
        img_array = np.frombuffer(value_buf, np.uint16)
